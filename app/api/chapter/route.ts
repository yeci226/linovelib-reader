...
export async function GET(req: NextRequest) {
  ...
  // bili_novel_packer always fetches from www.bilinovel.com (CN), not tw.linovelib.com (TW).
  // CN has no Cloudflare and the same shuffle mechanism. Transparently rewrite TW → CN.
  const url = rawUrl
    .replace("www.bilinovel.com", "tw.linovelib.com")
    .replace("cn.linovelib.com", "tw.linovelib.com");
  ...
  try {
    ...
    while (currentUrl && pageCount < MAX_PAGES) {
      ...
      const { title: pageTitle, content, nextPageUrl, nextChapterUrl: nc, prevChapterUrl: pc } = await extractPage(html, currentUrl);
      if (pageCount === 0 && pageTitle) title = pageTitle;
      if (pageCount === 0 && pc) prevChapterUrl = pc;
      if (nc) nextChapterUrl = nc;
      allContent += content + "\n\n";
